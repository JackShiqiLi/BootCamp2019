...
model; 
	...
	(c - b * c(-1)) ^ (-1 / eeis) = bbeta * ((c(+1) - b * c) ^ (-1 / eeis)) * (1 - ddelta + r(+1));
	
	// 2. First order equation for labor-leisure
	w * ((c - b * c(-1)) ^ (-1 / eeis))			= cchi * (l ^ (1 / eeta));
...
stoch_simul(hp_filter = 1600, irf = 80, order = 3) logY logC logI logL logW r_annual;
